...
import io.github.shymoy.termagent.toolresult.ReplacementRecordsIO;
import io.github.shymoy.termagent.toolresult.ToolResultBudget;
...
public class Agent {
    ...
    private final io.github.shymoy.termagent.compact.ContextCompactor.AutoCompactTrackingState compactTracking =
            new io.github.shymoy.termagent.compact.ContextCompactor.AutoCompactTrackingState();

    /**
     * 上下文压缩使用的真实 token 基准。每轮流结束后根据服务商返回的 usage 更新；
     * 第一轮尚无 usage 时为空，压缩器会暂时使用字符数估算。
     */
    private io.github.shymoy.termagent.compact.ContextCompactor.UsageAnchor usageAnchor;
     ...
     */
    private final io.github.shymoy.termagent.compact.RecoveryState recoveryState =
            new io.github.shymoy.termagent.compact.RecoveryState();

    public io.github.shymoy.termagent.compact.RecoveryState getRecoveryState() { return recoveryState; }
    ...
    public io.github.shymoy.termagent.filehistory.FileHistory getFileHistory() { return fileHistory; }
    ...
    private void agentLoop(ConversationManager conv, BlockingQueue<AgentEvent> queue) {
        ...
        for (int iteration = 1; ; iteration++) {
            ...
            Path preCompactSessionDir = Paths.get(workDir == null ? "." : workDir, ".termagent/session");
            ...
            try {
                ...
                String compactMsg = io.github.shymoy.termagent.compact.ContextCompactor.manage(
            ...
            Path sessionDir = Paths.get(workDir == null ? "." : workDir, ".termagent/session");
            ...
            if (streamError) {
                ...
                if (lastErr != null && (lastErr.contains("context") || lastErr.contains("too long")
                    ...
                    if (contextRetries < 3) {
                        ...
                        Path forceSessionDir = Paths.get(workDir == null ? "." : workDir, ".termagent/session");
                        ...
                        try {
                            ...
                            io.github.shymoy.termagent.compact.ContextCompactor.forceCompact(
            ...
            if (turnInput > 0 || turnOutput > 0 || turnCacheRead > 0 || turnCacheCreation > 0) {
                ...
                usageAnchor = new io.github.shymoy.termagent.compact.ContextCompactor.UsageAnchor(
                        baseline, conv.size());
            }
