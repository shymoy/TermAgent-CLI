...
/**
 * 加载项目和用户级别的指令文件（TERMAGENT.md / AGENTS.md），支持 @include 递归展开。
 *
 * <p>发现顺序（越靠后优先级越高，模型注意力优先关注后出现的内容）：
 * <ol>
 *   <li>用户全局：~/.termagent/TERMAGENT.md、~/.termagent/AGENTS.md</li>
 *   <li>项目级：从 git root 沿目录树向下到 workDir，每个目录的 TERMAGENT.md 和 AGENTS.md</li>
 *   <li>workDir/.termagent/INSTRUCTIONS.md（兼容旧版）</li>
 *   <li>workDir/TERMAGENT.local.md（私有本地覆盖）</li>
 * </ol>
 *
 * <p>@include 指令：
 * <ul>
...
public final class InstructionLoader {
    ...
    static List<InstructionSource> discoverInstructions(String workDir) {
        ...
        // 1. 用户全局指令
        addSource(sources, seen, AppPaths.legacyUser("MEWCODE.md"));
        addSource(sources, seen, AppPaths.legacyUser("AGENTS.md"));
        addSource(sources, seen, AppPaths.user("AGENTS.md"));
        addSource(sources, seen, AppPaths.user("TERMAGENT.md"));
        ...
        for (String dir : projectInstructionDirs(workDir)) {
            ...
            addSource(sources, seen, Path.of(dir, "TERMAGENT.md"));
        }

        // 3. 兼容旧版 INSTRUCTIONS.md
        addSource(sources, seen, AppPaths.legacyProject(Path.of(workDir), "INSTRUCTIONS.md"));
        addSource(sources, seen, AppPaths.project(Path.of(workDir), "INSTRUCTIONS.md"));

        // 4. 本地私有覆盖
        addSource(sources, seen, Path.of(workDir, "MEWCODE.local.md"));
        addSource(sources, seen, Path.of(workDir, "TERMAGENT.local.md"));
