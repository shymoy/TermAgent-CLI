...
public class AgentTool implements Tool {
     ...
     */
    private io.github.shymoy.termagent.worktree.WorktreeManager worktreeManager;
     ...
     */
    private io.github.shymoy.termagent.teams.TeamManager teamManager;
     ...
     */
    private io.github.shymoy.termagent.toolresult.ContentReplacementState parentReplacementState;

    public void setParentReplacementState(io.github.shymoy.termagent.toolresult.ContentReplacementState state) {
        this.parentReplacementState = state;
    }

    public void setWorktreeManager(io.github.shymoy.termagent.worktree.WorktreeManager worktreeManager) {
        this.worktreeManager = worktreeManager;
    }

    public void setTeamManager(io.github.shymoy.termagent.teams.TeamManager teamManager) {
        ...
        for (var msg : parent.getMessages()) {
            if (msg.getToolUses() != null && !msg.getToolUses().isEmpty()
                ...
                var placeholders = msg.getToolUses().stream()
                        .map(tu -> new io.github.shymoy.termagent.conversation.ToolResultBlock(
    ...
    private ToolResult runSync(SubAgentSpec spec, String description, String prompt, String modelOverride, String isolation) {
        ...
        io.github.shymoy.termagent.worktree.AgentWorktree.Result wtResult = null;
        if ("worktree".equals(isolation) && worktreeManager != null) {
            ...
            try {
                wtResult = io.github.shymoy.termagent.worktree.AgentWorktree.create(
                        slug, worktreeManager.getProjectRoot(), worktreeManager.getSymlinkDirs());
                subAgent.setWorkDir(wtResult.worktreePath());
                // 将工作树通知插入提示中
                String notice = io.github.shymoy.termagent.worktree.AgentWorktree.buildNotice(
        ...
        while (true) {
            ...
            switch (event) {
                ...
                case AgentEvent.LoopComplete lc -> {
                    ...
                    String wtInfo = "";
                    if (wtResult != null) {
                        if (io.github.shymoy.termagent.worktree.WorktreeChanges.hasChanges(
                            ...
                            io.github.shymoy.termagent.worktree.AgentWorktree.remove(
    ...
    private ToolResult runAsTeammate(SubAgentSpec spec, String teamName,
        ...
        ToolRegistry subRegistry = ToolFilter.filterForAgent(parentRegistry, spec);
        // 为队友添加协调工具
        subRegistry.register(new io.github.shymoy.termagent.teams.TeamTools.SendMessageTool(teamManager, memberName));
        ...
        // 构建附录
        String addendum = io.github.shymoy.termagent.teams.TeammateRunner.buildTeammateAddendum(
        ...
        if ("worktree".equals(isolation) && worktreeManager != null) {
            try {
                ...
                new java.security.SecureRandom().nextBytes(rndBytes);
                String slug = "agent-a" + java.util.HexFormat.of().formatHex(rndBytes).substring(0, 7);
                var wtResult = io.github.shymoy.termagent.worktree.AgentWorktree.create(
                        slug, worktreeManager.getProjectRoot(), worktreeManager.getSymlinkDirs());
        ...
        // 生成队友
        try {
            var spawnResult = io.github.shymoy.termagent.teams.SpawnDispatcher.spawnTeammate(
                    new io.github.shymoy.termagent.teams.SpawnDispatcher.SpawnConfig(
