...
/**
 * 从 .termagent/commands/ 目录加载自定义 Markdown 命令文件。
...
public final class CommandLoader {
    ...
    /**
     * 合并用户全局和项目级的文件命令。
     * 搜索路径：1. ~/.termagent/commands/  2. $workDir/.termagent/commands/
     * 后者覆盖前者同名命令。
     */
    public static List<Command> loadUserCommands(String workDir) {
        List<String> dirs = new ArrayList<>();

        AppPaths.userLayers("commands").forEach(path -> dirs.add(path.toString()));
        AppPaths.projectLayers(Path.of(workDir), "commands").forEach(path -> dirs.add(path.toString()));
